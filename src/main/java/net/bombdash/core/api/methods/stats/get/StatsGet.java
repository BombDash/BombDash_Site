...
public class StatsGet extends AbstractExecutor<StatsGetRequest, PlayerList<StatsGet.StatsGetData>> {
    ...
    public PlayerList<StatsGetData> execute(StatsGetRequest json, BombDashUser user) throws MethodExecuteException {
        if (json.getType() == null)
            throw new MethodExecuteException(MethodExecuteExceptionCode.FIELD_MISSING, "Missing required params");
        int rPage;
        ...
        if (count == null) {
            throw new MethodExecuteException(MethodExecuteExceptionCode.WRONG_SORT, "Score type " + json.getType() + " does not exists");
        }
        SqlRowSet set;
